...
load(fullfile('TMP','param'));
param0=param;
roi=param0.roi;
tic;
load(fullfile('TMP',sprintf('%d_param',nmod)),'param');
pscale=2^(iscale-1);
sbasis=param.basis;
...
switch sbasis
    ...
    case 'rbt'
        ...
        phiy(:,2)=1;
            ...
            Xi=Xi(:);
            Yi=Yi(:);
            save(fullfile('TMP',sprintf('%d_phix_%d',nmod2,10*(iscale-1))),'phix','Xi','Yi','wdetJ','sizeim');
            save(fullfile('TMP',sprintf('%d_phiy_%d',nmod2,10*(iscale-1))),'phiy','sizeim');
        end
       [Yi,Xi]=meshgrid(1:sizeim(2),1:sizeim(1));
        siz=size(Xi);
        phix=zeros(prod(siz), 2);
    ...
    case 'nurbs'
        ...
        if iscale>1
            ...
            Xi=Xi(:);
            Yi=Yi(:);
            phi1=CreateNURBSBasis(mesh_file,sizeim,p,pscale);
            phi0=sparse(size(phi1,1),size(phi1,2));
            phix=[phi1,phi0];
            save(fullfile('TMP',sprintf('%d_phix_%d',nmod2,10*(iscale-1))),'phix','Xi','Yi','wdetJ','sizeim');
            phiy=[phi0,phi1];
        ...
        [Yi,Xi]=meshgrid((1:sizeim(2)),(1:sizeim(1)));
        if ( (iscale ==1) && strcmp(type_nurbs,'c0') )
            phi1=CreateNURBSBasisCo(mesh_file,sizeim,p);
        else
           % [phi1,dphi1]=CreateNURBSandDersBasis(mesh_file,sizeim,p);
            				 phi1=CreateNURBSBasis(mesh_file,sizeim,p);
%             dphi0=sparse(size(dphi1,1),size(dphi1,2));
%             dphix=[dphi1,dphi0];
%             dphiy=[dphi0,dphi1];
%             clear dphi0 dphi1
        end
...
save(fullfile('TMP',sprintf('%d_phiy_%d',nmod2,iscale-1)),'phiy','sizeim','Nddl_tot');
save(fullfile('TMP',sprintf('%d_phix_%d',nmod2,iscale-1)),'phix','Xi','Yi','wdetJ','inde','sizeim','Nddl_tot');
